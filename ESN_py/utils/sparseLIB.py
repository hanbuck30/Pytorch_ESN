...
class sparseESN(nn.Module):
    def __init__(self, args):
        ...
        W_in = self._create_sparse_reservoir(1 + self.n_feature, self.resSize, self.sparsity, self.spectral_radius, "Win", self.input_scaling, self.weight_scaling)
        self.Win = nn.Parameter(W_in, requires_grad=False).to(device)
        
        # Create sparse reservoir
        W_data = self._create_sparse_reservoir(self.resSize, self.resSize, self.sparsity, self.spectral_radius, "W", self.input_scaling, self.weight_scaling)
